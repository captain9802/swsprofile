    ...
    async submitBlog() {
      ...
      try {
        if (this.blog && this.realToken) {
          ...
          const response = await axios.post(`https://sws-portfolio.com/blog/update/${this.blog.id}`, formData, {
            headers: { 'Content-Type': 'multipart/form-data','Authorization': `Bearer ${this.realToken}` }
          });
          toast.success(response.data.message);
          this.$emit('close-dialog');
        } else {
          const response = await axios.post('https://sws-portfolio.com/blog/add', formData, {
            headers: { 'Content-Type': 'multipart/form-data', 'Authorization': `Bearer ${this.realToken}` }
          });
          toast.success(response.data.message);
        }
